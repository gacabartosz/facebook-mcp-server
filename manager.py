...
class Manager:
    ...
    def filter_negative_comments(self, comments: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        return [
            c for c in comments.get("data", [])
            if any(k in (c.get("message") or "").lower() for k in keywords)
        ]
    ...
    def get_number_of_likes(self, post_id: str) -> int:
        data = self.api._request("GET", post_id, {"fields": "likes.summary(true)"})
    ...
    def bulk_delete_comments(self, comment_ids: list[str]) -> list[dict[str, Any]]:
        ...
        for cid in comment_ids:
            try:
                ...
                results.append({"comment_id": cid, "success": True, "result": res})
            except FacebookAPIError as e:
                results.append({"comment_id": cid, "success": False, "error": str(e)})
        return results
